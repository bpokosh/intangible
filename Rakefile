...
deploy_default  = "s3"
s3_bucket       = "www.intangiblecreations.com"
...
task :new_post, :title, :date do |t, args|
  ...
  if date.nil?
    if args.date
      ...
      date = get_stdin("Enter a date for your post (leave blank for current date): ")
    end
  end
  raise "### You haven't set anything up yet. First run `rake install` to set up an Octopress theme." unless File.directory?(source_dir)
  mkdir_p "#{source_dir}/#{posts_dir}"
  filename = "#{source_dir}/#{posts_dir}/#{date == '' ? Time.now.strftime('%Y-%m-%d') : date}-#{title.to_url}.#{new_post_ext}"
  ...
  open(filename, 'w') do |post|
    ...
    post.puts "date: #{date == '' ? Time.now.strftime('%Y-%m-%d %H:%M:%S %z') : date}"
